...
* Последней командой **_Console.WriteLine($"[{String.Join(", ", stringArray)}] -> [{String.Join(", ", newStringArray)}]")_** в терминале выводятя в удобном виде все элементы первоначального массива строк и элементы нового массива.